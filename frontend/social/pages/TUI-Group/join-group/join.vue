...
<view class="TUI-Create-conversation-container">
  ...
  <view class="tui-search-area">
    <view class="tui-search-bar">
      ...
      <input class="tui-search-bar-input" :value="groupID" placeholder="请输入群ID" @input="groupIDInput" @confirm="searchGroupByID" @blur="searchGroupByID">
    </view>
  </view>
  <view class="tui-person-to-invite" v-if="searchGroup.groupID">
    <image @tap="handleChoose" class="tui-normal-choose" :src="isChoose ? '/static/static/assets/single-choice-hover.svg' : '/static/static/assets/single-choice-normal.svg'"></image>
...
export default {
  ...
  methods: {
    ...
    searchGroupByID() {
      uni.$TUIKit.searchGroupByID(this.groupID).then(imResponse => {
